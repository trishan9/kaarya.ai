...
export function SavedHero({
  ...
  return (
    <section className="relative overflow-hidden rounded-2xl bg-linear-to-r from-[#00629F]/80 to-[#00629F] px-4 py-8 text-white sm:px-8">
      <div className="pointer-events-none absolute left-0 bottom-0 h-10 w-80 rounded-tr-2xl bg-white/10" />
      <div className="pointer-events-none absolute right-0 top-0 h-16 w-56 rounded-bl-2xl rounded-tr-2xl bg-white/12" />
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {stats.map((stat) => (
            <div
              key={stat.id}
              className="rounded-xl border border-white/20 bg-white/12 px-3 py-2.5 backdrop-blur-sm"
            >
